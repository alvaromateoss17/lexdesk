...
import {
  ...
  detectarMapeoAutomatico, CAMPOS_EXPEDIENTE,
...
function Paso1({ onDatos, onClose }) {
  ...
  function procesarPegado() {
    ...
    if (!resultado.cabeceras.length) { setError('No se detectaron columnas.'); return }
    onDatos(resultado)
  }

  const tarjetas = [
    { id: 'archivo',   icono: FileSpreadsheet, titulo: 'Subir Excel o CSV', desc: 'Sube un archivo .xlsx, .xls o .csv con tus expedientes' },
    { id: 'pegar',     icono: ClipboardPaste,  titulo: 'Pegar desde Excel', desc: 'Copia celdas directamente desde Excel y pégalas aquí' },
    { id: 'plantilla', icono: Download,        titulo: 'Descargar plantilla', desc: 'Descarga nuestra plantilla Excel ya preparada' },
  ]
  ...
  return (
    <div style={{ padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        {tarjetas.map(t => {
          const Ic = t.icono; const sel = metodo === t.id
          return (
            <div key={t.id} onClick={() => setMetodo(metodo === t.id ? null : t.id)} style={{
              border: `2px solid ${sel ? '#4F7EFF' : 'var(--border)'}`, borderRadius: 10, padding: '18px 16px',
              cursor: 'pointer', background: sel ? 'rgba(79,126,255,0.06)' : 'var(--surface-2)', transition: 'all 0.15s',
            }}>
          ...
          >
            {cargando ? (
              ...
              <>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-3)' }}>.xlsx, .xls, .csv</div>
              </>
            )}
          </div>
        </div>
      ...
      {metodo === 'pegar' && (
        <div>
          ...
          <textarea
            ...
            placeholder="Referencia&#9;Tipo&#9;Cliente&#9;Estado&#10;EXP-001&#9;Divorcio&#9;María García&#9;activo"
            style={{ ...inputStyle, height: 120, resize: 'vertical', padding: '8px 10px', fontFamily: 'monospace', fontSize: 12 }}
          />
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10 }}>
            <button style={btnPri} onClick={procesarPegado}>Siguiente <ChevronRight size={14} /></button>
          </div>
        </div>
      )}
      ...
      {metodo === 'plantilla' && (
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', background: 'rgba(79,126,255,0.06)', border: '1px solid rgba(79,126,255,0.2)', borderRadius: 8, padding: '14px 16px' }}>
          ...
          <button style={btnPri} onClick={descargarPlantillaExpedientes}><Download size={14} /> Descargar</button>
        </div>
      )}
...
function Paso2({ cabeceras, filas, onConfirmar, onAtras }) {
  const [mapeo, setMapeo] = useState(() => detectarMapeoAutomatico(cabeceras, 'expediente'))
  ...
  return (
    <div style={{ padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 24px 1fr', gap: '8px 0', alignItems: 'center' }}>
          ...
          {cabeceras.map(cab => (
            <>
              ...
              <select key={`sel-${cab}`} value={mapeo[cab] || ''} onChange={e => setMapeo(prev => ({ ...prev, [cab]: e.target.value }))}
                style={{ ...selectStyle, border: mapeo[cab] ? '1px solid rgba(79,126,255,0.4)' : '1px solid var(--border-2)' }}>
                <option value="">— No importar —</option>
                {CAMPOS_EXPEDIENTE.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
              </select>
            </>
          ))}
        </div>
      ...
      <div>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>Previsualización</div>
              ...
              <tr style={{ background: 'var(--surface-2)' }}>
                {cabeceras.filter(c => mapeo[c]).map(c => (
                  <th key={c} style={{ padding: '8px 12px', textAlign: 'left', color: 'var(--text-2)', fontWeight: 500, borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap' }}>
                    {CAMPOS_EXPEDIENTE.find(f => f.id === mapeo[c])?.label || c}
                  </th>
                ))}
              </tr>
            </thead>
        ...
        <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 6 }}>{filas.length} registros totales</div>
      </div>

      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <button style={btnSec} onClick={onAtras}>← Atrás</button>
        <button style={btnPri} onClick={() => onConfirmar(mapeo)}>Siguiente <ChevronRight size={14} /></button>
      </div>
    </div>
  )
}
...
function Paso3({ filas, mapeo, expedientesExistentes, onImportar, onAtras, importando }) {
  const procesados = filas.map(fila => {
    const datos   = mapearFila(fila, mapeo)
    const errores = validarExpediente(fila, mapeo)
    const esDup   = expedientesExistentes.some(e =>
      datos.ref && e.ref && e.ref.toLowerCase() === datos.ref.toLowerCase()
    )
    ...
    <div style={{ padding: 28, display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        ...
        <div style={{ padding: '14px 16px', borderRadius: 8, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 2 }}>ℹ️ Referencias duplicadas</div>
        </div>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 100, overflowY: 'auto' }}>
            {conError.map((p, i) => (
              <div key={i} style={{ fontSize: 12, color: 'var(--text-2)' }}>
                <span style={{ color: 'var(--text-3)' }}>Fila {i + 2}:</span> {p.errores.join(', ')}
              </div>
            ))}
          </div>
        </div>
      ...
      {duplics.length > 0 && (
        <div style={{ background: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.15)', borderRadius: 8, padding: '12px 14px', fontSize: 13, color: '#FCD34D' }}>
          {duplics.length} expediente(s) tienen referencias ya existentes y serán omitidos.
        </div>
      )}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
        ...
        <button
          style={{ ...btnPri, opacity: validos.length === 0 || importando ? 0.6 : 1, cursor: validos.length === 0 || importando ? 'not-allowed' : 'pointer' }}
          disabled={validos.length === 0 || importando}
          onClick={() => onImportar(validos.map(p => p.datos))}
        >
          {importando ? <><Loader size={14} style={{ animation: 'spin 1s linear infinite' }} /> Importando…</> : `Importar ${validos.length} expedientes`}
        </button>
      </div>
    </div>
  )
...
export default function ImportarExpedientesModal({ onClose, onImportados, expedientesExistentes = [], clientesExistentes = [] }) {
  ...
  function handleDatos({ cabeceras, filas }) {
    setCabeceras(cabeceras); setFilas(filas); setPaso(2)
  }
  function handleMapeo(mapeo) {
    setMapeo(mapeo); setPaso(3)
  }
    ...
    <div style={overlay} onClick={onClose}>
      <div style={modal} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 28px 0' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700 }}>Importar expedientes</h2>
            <p style={{ margin: '2px 0 0', fontSize: 13, color: 'var(--text-2)' }}>Importa múltiples expedientes desde Excel o CSV</p>
          </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {paso === 3 && (
            <Paso3
              filas={filas} mapeo={mapeo}
              expedientesExistentes={expedientesExistentes}
              onImportar={handleImportar} onAtras={() => setPaso(2)} importando={importando}
            />
          )}
        </div>
      </div>
